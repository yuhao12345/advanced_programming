...
int main(const int argc, const char** argv) {
    ...
    int i,tn;
    int   nParticles=4;      /* number of particles */
    int NT=2;  /*number of threads*/
    /*   nParticles = atoi(argv[1]);
       NT  =   atoi(argv[2]);*/

    const float dt    = 0.01f; /* time step   */
    const int nIters  = 2;   /* number of steps in simulation */
    ...
    for (int iter = 1; iter <= nIters; iter++) {
        ...
        for (tn = 0; tn < NT; tn++) {
            ...
            pa[tn].start = i * pa[i].nl;
            pa[tn].nParticles = nParticles;
            pthread_create(&threads[tn], NULL, *calc_force,
                           (void *) &pa[tn]);   /* compute interparticle forces and update vel */
        }
